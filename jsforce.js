        ...
        conn.bulk.load(sObjectName, CRUDOperation, records, function(err, rets) {
          ...
          for (var i=0; i < rets.length; i++) {
            ...
            if (rets[i].success) {
              ...
              errors.push(queryObj['error'] = "#" + (i+1) + " error occurred, message = " + rets[i].errors.join(', '));
            }
          }
          results = {ids, errors};
          resolve(results);
    ...
    return new Promise((resolve, reject) => {
      insertRecords('Account', 'insert', accountsToinsert).then((results) => {
        if (results.ids.length > 0) {
          ...
          getRecordsByIds('Account', results.ids, fields).then((records) => {
            if (records.length > 0) {
  ...
  const insertContacts = (contactsToInsert) => {
    return new Promise((resolve, reject) => {
      insertRecords('Contact', 'insert', contactsToInsert).then((results) => {
        if (results.ids.length > 0) {
          let fields = { Id: 1, au_External_ID__c: 1 }
          getRecordsByIds('Contact', results.ids, fields)
            .then((records) => {
              if (records.length > 0) {
    ...
    return new Promise((resolve, reject) => {
      insertRecords('Case', 'insert', casesToInsert).then((results) => {
        if (results.ids.length > 0) {
          ...
          getRecordsByIds('Contact', results.ids, fields).then((records) => {
            if (records.length > 0) {
    ...
    return new Promise((resolve, reject) => {
      insertRecords('Opportunity', 'insert', opportunitiesToInsert).then((results) => {
        if (results.ids.length > 0) {
          ...
          getRecordsByIds('Opportunity', results.ids, fields).then((records) => {
            if (records.length > 0) {
